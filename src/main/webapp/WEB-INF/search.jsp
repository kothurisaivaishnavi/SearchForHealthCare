...
<header>
	...
	<form:form>
	<table >  
<tr><th>Name</th><th>Mobile no.</th><th>Service Name</th><th>Organization/company</th><th>Location</th><th>Address</th><th>More details</tr>  
   <c:forEach var="search" items="${slist}">   
   <tr> 
   <td>${search.name}</td>  
   <td>${search.number}</td>    
   <td>${search.serviceName}</td>  
   <td>${search.bussinessname}</td> 
   <td>${search.address}</td> 
   <td>${search.location}</td>  
   <td><a href="/address/${search.serviceId}">Click here</a></td> 
   </tr>  
   </c:forEach>  
   </table> 
   </form:form>
